...
for other_image_path in other_images:
    ...
    ssd_match_orb = matchDescriptorsSSD(ref_descriptor_with_orb_harris, test_descriptor_with_orb_harris)
    ssd_match_orb = sorted(ssd_match_orb, key=lambda x: x.distance)
    gray_ssd_match_orb = cv2.drawMatches(gray_bernie_image, ref_keypoints_with_orb_harris, gray_testing_image, test_keypoints_with_orb_harris, ssd_match_orb[:80], outImg=None, flags=2)
    # cv2.imwrite(f"OUTPUT/ORB/{other_image_path}_orb_match.jpg", gray_ssd_match_orb)

    #WITH HARRIS CORNER
    test_response, test_harris_keypoints = harrisCornerDetection(gray_bernie_image)
    test_harris_keypoints, test_descriptors = orb.compute(gray_bernie_image, test_harris_keypoints )
    result_test_keypoints = cv2.drawKeypoints(gray_bernie_image, test_harris_keypoints, None, color=(0,255,0), flags=0)
    ...
    # cv2.imwrite(f"OUTPUT/HARRIS/{other_image_path}_harris_match.jpg", gray_ssd_match)

    #WITH RATIO TEST
    ratio_match = matchDescriptorsRatio(ref_descriptors, test_descriptors)
    ratio_match = sorted(ratio_match, key=lambda x: x.distance)
    gray_ratio_match = cv2.drawMatches(gray_bernie_image, ref_keypoints, gray_testing_image, test_harris_keypoints, ratio_match[:80], outImg=None, flags=2)
    # cv2.imwrite(f"OUTPUT/RATIO/{other_image_path}_harris_ratio_match.jpg", gray_ratio_match)

    # cv2.imwrite(f"{other_image_path}_keypoints.jpg", test_gray_keypoint)
    # cv2.imwrite(f"{other_image_path}_match.jpg", gray_ssd_match)


# threshold_values = [0.01,0.06,0.1,0.5,0.7,0.9]  # Adjust the range as needed
# harris_keypoints_counts = []
# orb_keypoints_counts = []

# # Loop over the range of threshold values
# for thresh in threshold_values:
#     # Harris Corner Detection
#     _, harris_keypoints = harrisCornerDetection(gray_bernie_image, sigma=1.7, k=0.06, thresh=thresh)
#     harris_keypoints_counts.append(len(harris_keypoints))

#     # ORB Detection
#     # keypoints_with_orb_harris, descriptor_with_orb_harris, _, _ = cornerDetectionWithORB(gray_bernie_image)
#     # orb_keypoints_counts.append(len(keypoints_with_orb_harris))

# print("harris_keypoints_counts", harris_keypoints_counts)
# print("orb_keypoints_counts", orb_keypoints_counts)

# plt.figure(figsize=(10, 5))
# plt.plot(threshold_values, harris_keypoints_counts, label='Harris Corner Keypoints')
# # plt.plot(threshold_values, orb_keypoints_counts, label='ORB Keypoints', marker='x')
# plt.xlabel('Threshold')
# plt.ylabel('Number of Keypoints')
# plt.title('Threshold vs Keypoints')
# # plt.legend()
# plt.grid(True)
# plt.savefig("ThresholdvsKeypoints_MYOWN")
