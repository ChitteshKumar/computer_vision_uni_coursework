...
def simpleThresholding(threshold_value, image):
    thresholded_result = np.zeros_like(image).astype(np.float64)
...
#--------------SIMPLE THRESHOLDING - WITH AVERAGE SMOOTHING & SOBEL--------------
...
# #edge strength after combined image  
gaussian_gradient_magnitude = np.sqrt((gaussian_gradient_y_image.astype(np.float64) **2) + (gaussian_gradient_y_image.astype(np.float64) **2) )
...
#--------------SIMPLE THRESHOLDING-----------------
threshold_value = 21 # 18,25,35,30,40,50,60
# thresholded_image = adaptiveThresholding(threshold_value, gaussian_gradient_magnitude)
